...
Analyst ─► Product ─► Architect ─► Scrum Master ─► Developer ─► QA
 forks      PRD        design        task list       code      verdict
```
...
- **Independence is the point.** QA is a DIFFERENT subagent than Developer —
  ...
  to pay for roles.
- **"Verifier is not an oracle" still holds.** QA findings are hypotheses; accept
  only those tied to a source (Step 5). A role title does not make a claim true.
...
- **Collapse freely.** On a medium task you may merge roles (e.g. Architect +
  Scrum Master in one subagent). Roles are a lens, not mandatory headcount.
